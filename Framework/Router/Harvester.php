...
class Harvester
{
    ...
    public function harvestRoutes(): array {
        ...
        foreach ($controllers as $controllerName => $controller) {
            if (empty($controller->methods['__construct'])) {
                $controller->methods['__construct'] = new stdClass();
                $controller->methods['__construct']->type = '';
                $controller->methods['__construct']->route = '';
                $controller->methods['__construct']->name = '';
                $controller->methods['__construct']->authRequired = false;
                $controller->methods['__construct']->authLevel = 'token';
                $controller->methods['__construct']->regex = '';
                $controller->methods['__construct']->annotations = array(
                    'Route' => new  stdClass(),
                );
                $controller->methods['__construct']->annotations['Route']->vars = array(
                    'type' => '',
                    'route' => '',
                    'name' => '',
                    'authRequired' => false,
                ...
                $controllerRoute = $controller->methods['__construct'];
            } else {
